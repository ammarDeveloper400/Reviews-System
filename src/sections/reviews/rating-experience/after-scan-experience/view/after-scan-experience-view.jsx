...
import AfterScanExperienceSlider from '../after-scan-experience-slider';
// import StaffModal from '../barcode';

// ----------------------------------------------------------------------

export default function AfterScanExperienceView() {
    ...
    <>
      <Container maxWidth="xl" sx={{ pb: 4 }}>
        {isLoading ? (
          ...
          <>
            <AfterScanExperienceSlider interfaces={data?.interfaces} />
            {data?.interfaces?.interface3?.status && (
              <Box display="flex" justifyContent="center" mt="108px">
                <Button
                  LinkComponent={Link}
                  to={`/visit-social-media?store=${searchParams.get('store')}`}
                  variant="contained"
                  sx={{
                    background: '#FFC500',
                    color: '#000',
