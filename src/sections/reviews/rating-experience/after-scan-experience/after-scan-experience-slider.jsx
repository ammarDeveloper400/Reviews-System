...
const AfterScanExperienceSlider = ({ interfaces }) => {
  ...
  const handleRatingChange = (newRating, id) => {
    ...
    if (filteredExperience?.length > 0) {
      ...
      setTimeout(() => {
        navigate(`/visit-social-media?store=${searchParams.get('store')}`);
      }, 500);
    }
  };
  ...
  const handleSubmit = async () => {
    const payload = {
      customerExperienceID: experienceId,
      rating: ratings[experienceId] || 0,
      feedbackMethod: 'QR',
  ...
  return (
    <Box sx={{ position: 'relative', width: '90%', margin: '0 auto' }}>
      <Box display="flex" flexWrap="wrap" justifyContent="space-between" alignItems="center">
        <Button
          LinkComponent={Link}
          to={`/after-scan?store=${searchParams.get('store')}`}
          endIcon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="32"
        ...
        <Typography
          sx={{
            fontSize: { md: '56px', xs: '40px', sm: '30px' },
            color: '#000',
            textAlign: 'center',
            marginBottom: '20px',
          }}
              ...
              <Box key={index} sx={{ padding: '0 10px' }}>
                ...
                <Box display="flex" justifyContent="center">
                  <Rating
                    ...
                    precision={1}
                    // onChange={(e, newValue) => handleRating(newValue, experience?._id)}
                    value={ratings[experience?._id] || 0} // Get specific rating for this experience
                    onChange={(e, newValue) => handleRatingChange(newValue, experience?._id)}
                    emptyIcon={
                      <Iconify
            ...
            {/* <Box>
              <Box>
                <Typography sx={{ fontSize: '30px', textAlign: 'center', fontWeight: 600 }}>
                  Rate more Experiences?
                </Typography>
                <Box display="flex" gap="30px " mt="36px" justifyContent="center">
                  <Button
                    onClick={() => {
                      setOpen(false);
                      setOpen2(true);
...
export default AfterScanExperienceSlider;
